...
AND GROUP_ID IN ({$group})";
		...
		//echo $sql ; print_r($boundedParameter); die;
...
//here for deleting individual salary
    ...
    public function getAccCode($payId, $data, $branchCode, $groupId){
        ...
        from HRIS_ACC_CODE_MAP WHERE pay_id = $payId and company_code = ({$companyCode}) and branch_code = {$branchCode} and group_id = {$groupId}";
...
//        return $this->rawQuery($sql, $boundedParameter);
	...
	public function getDataForVoucherSubDetail($sheetNo,$employeeId,$accCode, $branchCode, $groupId){
        $companyCode = "select c.company_code from HRIS_SALARY_SHEET ss
        left join HRIS_COMPANY C ON( ss.company_id = c.company_id)
        where ss.sheet_no = {$sheetNo}";
        ...
        and acm.acc_code = {$accCode} and acm.branch_code = {$branchCode} and acm.group_id = {$groupId}";
//print_r($sql);die;
...
GROUP BY
    ...
    branch_code) A 
        ...
        left join hris_pay_setup PS on (PS.pay_id = a.pay_id)
        left join fa_chart_of_accounts_setup fac on (fac.acc_code = ACM.acc_code and fac.company_code = acm.company_code)
        where fac.company_code = ({$companyCode}) and acm.branch_code = '{$branchCode}' and A.total <> 0 and acm.group_id = $groupId) d
    ...
    public function insertIntoVoucherSubDetail($data,$employeeId,$indiEmployee){
        ...
        if($transactionType == 'CR'){
            ...
            values
            ('{$voucherNo}', '{$companyCode}','{$branchCode}','{$formCode}', ({$serialNo}), '{$accCode}', 'E' || '{$indiEmployee}', '{$transactionType}', 0, {$amount}, ({$createdBy}), trunc(sysdate), 'N', 'NRS', 1, ({$particulars}))";
        }else{
            $sql = "insert into fa_voucher_sub_detail 
            ...
            values
            ('{$voucherNo}', '{$companyCode}','{$branchCode}','{$formCode}', ({$serialNo}), '{$accCode}', 'E' || '{$indiEmployee}', '{$transactionType}', {$amount}, 0, ({$createdBy}), trunc(sysdate), 'N', 'NRS', 1 )";
            }
    ...
    public function insertIntoDoubleVoucher($data, $employeeId, $sheetDetails){
        ...
        values
        ('{$voucherNo}', trunc(sysdate),'{$serialNo}','{$accCode}', '{$transactionType}', {$amount}, '{$formCode}', '{$companyCode}', '{$branchCode}', ({$createdBy}), trunc(sysdate), 'N', 'Salary Sheet ' || ' {$sheetDetails['SHEET_NO']} ' || '(' || '{$sheetDetails['SALARY_TYPE_NAME']}' || ' Salary) of ' || '{$sheetDetails['MONTH_EDESC']}' || ' ' || '{$sheetDetails['FISCAL_YEAR_NAME']}' || ' ' || '{$payEdesc}', 'E', '{$sheetDetails['SALARY_TYPE_NAME']}' || ' Salary')";
        //print_r($sql);die;
        return $this->rawQuery($sql);
    }
