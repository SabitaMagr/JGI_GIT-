...
class EventApproveRepository extends HrisRepository {

    public function __construct(AdapterInterface $adapter) {
        parent::__construct($adapter, EventRequest::TABLE_NAME);
    }

    public function edit(Model $model, $id) {
        $temp = $model->getArrayCopyForDB();
        $this->tableGateway->update($temp, [EventRequest::REQUEST_ID => $id]);
//        $this->executeStatement("
//                DECLARE
//                  V_EVENT_ID HRIS_EMPLOYEE_EVENT_REQUEST.EVENT_ID%TYPE;
//                  V_START_DATE HRIS_EMPLOYEE_EVENT_REQUEST.START_DATE%TYPE;
//                  V_END_DATE HRIS_EMPLOYEE_EVENT_REQUEST.END_DATE%TYPE;
//                  V_EMPLOYEE_ID HRIS_EMPLOYEE_EVENT_REQUEST.EMPLOYEE_ID%TYPE;
//                  V_STATUS HRIS_EMPLOYEE_EVENT_REQUEST.STATUS%TYPE;
//                  V_REQUEST_ID HRIS_EMPLOYEE_EVENT_REQUEST.REQUEST_ID%TYPE:= {$id};
...
//                    V_EMPLOYEE_ID,
//                    V_STATUS
//                  FROM HRIS_EMPLOYEE_EVENT_REQUEST
...
///not inserted into table like HRIS_EMP_TRAINING_ATTENDANCE
//         $boundedParameter = [];
...
//                   V_REQUEST_ID HRIS_EMPLOYEE_EVENT_REQUEST.REQUEST_ID%TYPE:= :id;
//                   V_ASSIGNED CHAR(1 BYTE)                                    :=NULL;
//                   V_DURATION HRIS_EMPLOYEE_EVENT_REQUEST.DURATION%TYPE;
//                 BEGIN
//                   SELECT EVENT_ID,
//                     TRUNC( START_DATE ),
//                     TRUNC( END_DATE ),
...
//                         WHERE EVENT_ID=V_EVENT_ID AND EMPLOYEE_ID=V_EMPLOYEE_ID;
//                     END IF;
                ...
                BEGIN
                  ...
                  IF V_EVENT_ID IS NOT NULL THEN
                    SELECT (
                      ...
                      END)
                    INTO V_ASSIGNED
                    FROM HRIS_EMPLOYEE_EVENT_ASSIGN
                    WHERE EVENT_ID = V_EVENT_ID
                    AND EMPLOYEE_ID   =V_EMPLOYEE_ID;
...
-- TO INSERT INTO ASSIGNED IF NOT ASSIGNED
                    IF V_ASSIGNED    ='N' AND V_STATUS='AP' THEN
                      INSERT
                      INTO HRIS_EMPLOYEE_EVENT_ASSIGN
                        (
                          EVENT_ID,
                          EMPLOYEE_ID,
                          STATUS,
...
-- UPDATE IF ALREADY ASSIGNED
                    IF V_ASSIGNED    ='Y' AND V_STATUS='AP' THEN
                      UPDATE HRIS_EMPLOYEE_EVENT_ASSIGN
                        SET STATUS='E',MODIFIED_DATE=TRUNC(SYSDATE) 
                        WHERE EVENT_ID=V_EVENT_ID AND EMPLOYEE_ID=V_EMPLOYEE_ID;
                    END IF;
...
", $boundedParameter);
    ...
    public function fetchById($id) {
        ...
        $select->columns([
            ...
            new Expression("TR.EVENT_ID AS EVENT_ID"),
            new Expression("(CASE WHEN TR.EVENT_ID IS NULL THEN TR.TITLE ELSE T.EVENT_NAME END) AS TITLE"),
            new Expression("TR.DESCRIPTION AS DESCRIPTION"),
            new Expression("TR.DURATION AS DURATION"),
            new Expression("TR.EVENT_TYPE AS EVENT_TYPE"),
            new Expression("(CASE WHEN TR.EVENT_TYPE = 'CC' THEN 'Company Contribution' ELSE 'Personal' END) AS EVENT_TYPE_DETAIL"),
        ...
        $select->from(['TR' => EventRequest::TABLE_NAME])
            ->join(['T' => Events::TABLE_NAME], "T." . Events::EVENT_ID . "=TR." . EventRequest::EVENT_ID, [
              Events::EVENT_CODE,
                "TRAINING_INSTRUCTOR_NAME" => new Expression("INITCAP(T.INSTRUCTOR_NAME)"),
                "EVENT_NAME" => new Expression("INITCAP(T.EVENT_NAME)"),
                ...
                "TRAINING_DURATION" => Events::DURATION,
                "TRAINING_EVENT_TYPE" => Events::EVENT_TYPE], "left")
    ...
    public function getPendingList($search) {
        $sql = "SELECT TR.REQUEST_ID,
                ...
                FROM HRIS_EMPLOYEE_EVENT_REQUEST TR
                LEFT JOIN HRIS_EVENT_MASTER_SETUP T
