function getCalendarDivString(a) {
    ...
    return b += '<span id="ndp-target-id" style="display:none"></span>',
            b += '<div class="ndp-corner-all ndp-header">',
            b += '<a href="javascript:void(0)" id="prev" title="Previous Month" class="ndp-prev"></a>',
            b += '<a href="javascript:void(0)" id="next" title="Next Month" class="ndp-next"></a>',
            b += '<span id="currentMonth"></span>',
            b += "</div>",
            b += '<div id="npd-table-div">',
            b += "<table>",
            b += '<tr class="ndp-days">',
            b += "<th>\u0906</th>",
            b += "<th>\u0938\u094b</th>",
...
function showNdpCalendarBox(a) {
    if (npdCalendarVisible)
        ...
        var c = $("#" + a).offset();
        $("#ndp-nepali-box").css("top", c.top + $("#" + a).outerHeight()),
                $("#ndp-nepali-box").css("left", c.left),
                showCalendar(b),
                npdCalendarVisible = !0
    }
}
function setSelectedDay(a) {
    var b = $("#ndp-target-id").html();
    $("#" + b).val(a),
            ndpAttr.ndpEnglishInput && $("#" + ndpAttr.ndpEnglishInput).val(BS2AD(a)),
            hideCalendarBox()
}
function showCalendar(a) {
    $("#ndp-nepali-box table").find("tr:gt(0)").remove(),
            "" === a ? $("#ndp-nepali-box table").append(getDateTable("")) : $("#ndp-nepali-box table").append(getDateTable(a)),
            "block" == $("#ndp-nepali-box").css("display") && $("#ndp-nepali-box").hide(),
...
function getDateTable(a) {
    ...
    if ("" === a) {
        var d = getNepaliDate();
        return b = getMonthParameters(d),
                c = getDateRows(b[0], b[1], b[2], b[3], b[4])
    }
    return b = getMonthParameters(a),
            c = getDateRows(b[0], b[1], b[2], b[3], b[4])
}
...
function getYearSelect(a, b, c) {
    ...
    for (i = Math.round(c / 2) > 0 && parseInt(b) - Math.round(c / 2) >= 2e3 ? parseInt(b) - Math.round(c / 2) : 2e3; i <= (Math.round(c / 2) > 0 && parseInt(b) + Math.round(c / 2) <= 2090 ? parseInt(b) + Math.round(c / 2) : 2090); i++)
        d += '<option value="' + i + '"' + (b == i ? " selected" : "") + ">" + englishNo2Nep(i) + "<li>";
    return d += "<select>"
}
function getMonthParameters(a) {
    ...
    ndpAttr.npdMonth && ndpAttr.npdYear ? $("#currentMonth").html(getMonthSelect(d, c) + getYearSelect(d, c, e)) : ndpAttr.npdMonth ? $("#currentMonth").html(getMonthSelect(d, c) + " " + englishNo2Nep(c)) : ndpAttr.npdYear ? $("#currentMonth").html(getNepaliMonth(d - 1) + " " + getYearSelect(d, c, e)) : $("#currentMonth").html(getNepaliMonth(d - 1) + "&nbsp;" + englishNo2Nep(c)),
            nYear = pYear = c,
            nMonth = parseInt(d, 10) + 1,
            nMonth > 12 && (nYear++,
                    nMonth = "01"),
            pMonth = parseInt(d, 10) - 1,
            pMonth < 1 && (pYear--,
                    pMonth = "12"),
...
function getDateRows(a, b, c, d, e) {
    for (var f = getNepaliDate(), g = f.split("-"), h = g[0], i = get2DigitNo(g[1]), j = get2DigitNo(g[2]), k = "", l = 0; a + b > l; l++) {
        ...
        var m = l - a + 1
                ...
                , o = "";
        o = c == h && get2DigitNo(d) == i && j == get2DigitNo(m) ? "ndp-selected" : m == e ? "ndp-current" : "ndp-date",
                a > l ? k += "<td></td>\n" : (k += "<td class='" + o + "'>",
                        k += "<a href='javascript:void(0)' onclick=\"setSelectedDay('" + n + "')\">" + englishNo2Nep(m) + "</a>",
                        k += "</td>\n"),
                l % 7 == 6 && (k += "</tr>\n")
    }
    return k
...
function hideCalendarBox(a) {
    a = "undefined" != typeof a ? a : !0,
            ...
            a && ndpAttr.onChange && ndpAttr.onChange()
}
function get2DigitNo(a) {
    return a = parseInt(a, 10),
            10 > a ? "0" + a.toString() : a.toString()
}
function getMonths() {
...
function NepaliDateConverter() {
    this.bs_date_eq = "09/17/2000",
            ...
            this.bs = [],
            this.bs[2e3] = new Array(30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31),
...
function add_ad_days(a, b) {
    ...
    return c.setDate(c.getDate() + b),
            ad_month = c.getMonth() + 1,
            ad_day = c.getDate(),
            c.getFullYear() + "-" + (ad_month < 10 ? "0" + ad_month.toString() : ad_month) + "-" + (ad_day < 10 ? "0" + ad_day.toString() : ad_day)
}
...
function bs2ad(a) {
    return days_count = this.count_bs_days(this.bs_date_eq, a),
            this.add_ad_days(this.ad_date_eq, days_count)
}
function ad2bs(a) {
    return days_count = this.count_ad_days(this.ad_date_eq, a),
            ...
            a.fn.nepaliDatePicker = function (b) {
                b = "undefined" != typeof b ? b : {},
                        ...
                        this.each(function () {
                            ...
                            a(this).addClass("ndp-nepali-calendar"),
                                    ndpData[c] = b,
                                    ndpAttr.onFocus !== !1 && a(this).attr("onfocus", "showNdpCalendarBox('" + c + "')"),
                                    ndpAttr.ndpTriggerButton && a(this).after('<button type="button" class="ndp-click-trigger ' + (ndpAttr.ndpTriggerButtonClass ? ndpAttr.ndpTriggerButtonClass : "") + '" onclick="showNdpCalendarBox(&quot;' + c + '&quot;)">' + (ndpAttr.ndpTriggerButtonText ? ndpAttr.ndpTriggerButtonText : "Pick Date") + "</button>")
                        }),
                        a("body").append(getCalendarDivString(ndpAttr)),
                        a(".ndp-nepali-calendar, #ndp-nepali-box").hover(function () {
                    mouse_is_inside = !0
                }, function () {
                    mouse_is_inside = !1
                }),
