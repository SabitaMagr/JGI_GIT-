create or replace TRIGGER "TRG_SYNC_EMPLOYEE" 
AFTER DELETE OR INSERT OR UPDATE
ON JWL_HRIS_APR5.HRIS_EMPLOYEES REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
DECLARE
...
BEGIN
	BEGIN
		SELECT COUNT (*) INTO V_COUNT FROM  HR_EMPLOYEE_SETUP
	...
	BEGIN
        SELECT COMPANY_CODE INTO V_COMPANY_CODE  FROM HRIS_COMPANY WHERE COMPANY_ID = :NEW.COMPANY_ID;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    IF V_COUNT = 0 THEN
		...
		, REPLACE(CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME),'  ',' ')
		,NVL(:NEW.NAME_NEPALI, CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '), :NEW.LAST_NAME))
		,'I'
		,'01.00'
		,'01'
		...
		SELECT COUNT(*)  INTO V_SUB_CODE FROM FA_SUB_LEDGER_SETUP WHERE TRIM(SUB_CODE)  =TRIM( V_EMPSUB_CODE);
		IF V_SUB_CODE = 0 THEN
			BEGIN
				INSERT INTO FA_SUB_LEDGER_SETUP(SUB_CODE, SUB_EDESC, SUB_NDESC, SUB_LEDGER_FLAG,
				COMPANY_CODE, CREATED_BY, CREATED_DATE, DELETED_FLAG)
				VALUES('E'||:NEW.EMPLOYEE_ID,
				NVL(:NEW.NAME_NEPALI, REPLACE(CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME),'  ',' ')),
				NVL(:NEW.NAME_NEPALI, CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME)),
				'E',
				V_COMPANY_CODE,
				'SYSTEM',
		...
		ELSE
			BEGIN
				UPDATE FA_SUB_LEDGER_SETUP SET
				SUB_EDESC =  REPLACE(CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME),'  ',' ')
				WHERE TRIM(SUB_CODE)= 'E'||:NEW.EMPLOYEE_ID;
			EXCEPTION
				WHEN OTHERS THEN
					NULL;
			END;
    ...
    ELSIF V_COUNT = 1 THEN
         BEGIN
            UPDATE  HR_EMPLOYEE_SETUP SET
            EMPLOYEE_EDESC = REPLACE(CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME),'  ',' ')
            ,EMPLOYEE_NDESC =  NVL(:NEW.NAME_NEPALI, CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME))
            ...
            ,LINK_SUB_CODE = 'E'||TO_CHAR(:NEW.EMPLOYEE_ID)
			...
			BEGIN
				UPDATE FA_SUB_LEDGER_SETUP SET
				SUB_EDESC = CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME),
				SUB_NDESC = NVL(:NEW.NAME_NEPALI, CONCAT(CONCAT(:NEW.FIRST_NAME||' ',:NEW.MIDDLE_NAME||' '),:NEW.LAST_NAME))
				WHERE TRIM(SUB_CODE) = 'E'||:OLD.EMPLOYEE_ID;
			EXCEPTION
				WHEN OTHERS THEN
					NULL;
			END;
