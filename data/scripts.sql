...
WITH OUT_TIME_QUERY AS
  (SELECT ATTENDANCE_TIME AS OUT_TIME,
    ATTENDANCE_DT,
    EMPLOYEE_ID,
    rownum rn1
  FROM
    ...
    WHERE EMPLOYEE_ID=1000370
    AND ATTENDANCE_DT=TO_DATE('10-May-2017','DD-MON-YYYY')
    ORDER BY ATTENDANCE_TIME
    )
  WHERE mod(rn,2)=0
  ) ,
  IN_TIME_QUERY AS
  (SELECT ATTENDANCE_TIME AS IN_TIME ,
    ATTENDANCE_DT,
    EMPLOYEE_ID ,
    rownum rn1
  FROM
...
SELECT IN_TIME_QUERY.IN_TIME,
  OUT_TIME_QUERY.OUT_TIME
FROM IN_TIME_QUERY,
  OUT_TIME_QUERY
WHERE IN_TIME_QUERY.rn1 = OUT_TIME_QUERY.rn1;
