...
BEGIN
  ...
  IF V_LINK_TRAVEL_TO_SYNERGY = 'Y' AND P_LINK_TO_SYNERGY = 'Y' THEN
    ...
    IF LOWER(V_REQUEST_TYPE) = 'ad' THEN
      HRIS_TRAVEL_ADVANCE(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),V_DR_ACC_CODE,V_CR_ACC_CODE,'TEST',V_ADVANCE_AMOUNT,'E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_ADVANCE_AMOUNT = V_EXPENSE_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_EQUAL(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),v_equal_dr_acc_code,V_DR_ACC_CODE,'TEST',V_EXPENSE_AMOUNT,'E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_ADVANCE_AMOUNT < V_EXPENSE_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_EXCESS(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),v_equal_dr_acc_code,V_DR_ACC_CODE,V_EXCESS_CR_ACC_CODE,V_ADVANCE_AMOUNT,V_EXPENSE_AMOUNT-V_ADVANCE_AMOUNT,'TEST','E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_ADVANCE_AMOUNT > V_EXPENSE_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_LESS(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),v_equal_dr_acc_code,V_LESS_DR_ACC_CODE,V_DR_ACC_CODE,V_ADVANCE_AMOUNT,V_ADVANCE_AMOUNT-V_EXPENSE_AMOUNT,'TEST','E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
