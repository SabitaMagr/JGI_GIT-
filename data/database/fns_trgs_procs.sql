...
BEGIN
  ...
  IF IS_END_OF_MONTH ='Y' THEN
    ...
    LOOP
      ...
      (
        V_MONTH_ID,V_DEDUCTION_RATE,penalty.EMPLOYEE_ID
      )
      ;
    END LOOP;
  END IF;
...
AS
  ...
  V_ALREADY_INSERTED CHAR(1 BYTE);
TYPE cur_typ
IS
  REF
  CURSOR;
  ...
  IF(V_ASSIGN_COUNT   =0) THEN
    ...
    IF P_EMPLOYEE_ID IS NOT NULL THEN
      V_QUERY        := V_QUERY||'AND EMPLOYEE_ID = '||P_EMPLOYEE_ID ;
    END IF;
  ELSE
    FOR assign IN
...
BEGIN
  ...
  IF V_LINK_TRAVEL_TO_SYNERGY = 'Y' AND P_LINK_TO_SYNERGY = 'Y' THEN
    ...
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_REQUESTED_AMOUNT = V_SETTLEMENT_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_EQUAL(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),V_DR_ACC_CODE,V_EQUAL_CR_ACC_CODE,'TEST',V_SETTLEMENT_AMOUNT,'E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_REQUESTED_AMOUNT < V_SETTLEMENT_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_EXCESS(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),V_DR_ACC_CODE,V_EQUAL_CR_ACC_CODE,V_EXCESS_CR_ACC_CODE,V_SETTLEMENT_AMOUNT,V_SETTLEMENT_AMOUNT-V_REQUESTED_AMOUNT,'TEST','E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    IF (LOWER(V_REQUEST_TYPE) ='ep') AND (V_REQUESTED_AMOUNT > V_SETTLEMENT_AMOUNT) THEN
      HRIS_TRAVEL_SETTLEMENT_LESS(V_COMPANY_CODE,V_FORM_CODE,TRUNC(SYSDATE),V_BRANCH_CODE,V_CREATED_BY,TRUNC(SYSDATE),V_DR_ACC_CODE,V_LESS_DR_ACC_CODE,V_EQUAL_CR_ACC_CODE,V_SETTLEMENT_AMOUNT,V_REQUESTED_AMOUNT-V_SETTLEMENT_AMOUNT,'TEST','E'||V_EMPLOYEE_ID,V_VOUCHER_NO);
    END IF;
    UPDATE HRIS_EMPLOYEE_TRAVEL_REQUEST
    SET VOUCHER_NO  = V_VOUCHER_NO
    WHERE TRAVEL_ID = P_TRAVEL_ID;
