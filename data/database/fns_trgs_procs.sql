...
DELETE FROM HRIS_CUST_CONTRACT_ATTENDANCE WHERE CONTRACT_ID=V_CONTRACT_ID AND MONTH_CODE_ID=V_MONTH_CODE_ID;


SELECT WORKING_CYCLE INTO V_WORKING_CYCLE
FROM HRIS_CUSTOMER_CONTRACT WHERE CONTRACT_ID=V_CONTRACT_ID;

SELECT FROM_DATE,TO_DATE INTO V_FROM_DATE,V_TO_DATE FROM HRIS_MONTH_CODE WHERE MONTH_ID=V_MONTH_CODE_ID;
...
SELECT V_TO_DATE -  V_FROM_DATE INTO V_DATE_DIFF FROM   dual;
  ...
  SELECT TO_CHAR(V_FROM_DATE+i,'D') INTO V_TEMP_DAY FROM DUAL;
   ...
   BEGIN
    ...
    FOR V_EMPLOYEES IN (SELECT * FROM HRIS_CUST_CONTRACT_EMP WHERE CONTRACT_ID=V_CONTRACT_ID AND MONTH_CODE_ID=V_MONTH_CODE_ID) LOOP
    DBMS_OUTPUT.PUT_LINE(V_EMPLOYEES.EMPLOYEE_ID);
    INSERT INTO HRIS_CUST_CONTRACT_ATTENDANCE (CONTRACT_ID,ATTENDANCE_DT,EMPLOYEE_ID,MONTH_CODE_ID,IN_TIME,OUT_TIME) VALUES (V_CONTRACT_ID,V_FROM_DATE+i,V_EMPLOYEES.EMPLOYEE_ID,V_MONTH_CODE_ID,V_EMPLOYEES.START_TIME,V_EMPLOYEES.END_TIME);
    END LOOP;
...
BEGIN
    FOR V_R_DATES IN (SELECT * FROM HRIS_CUST_CONTRACT_DATES WHERE CONTRACT_ID=V_CONTRACT_ID AND MANUAL_DATE BETWEEN V_FROM_DATE AND V_TO_DATE) LOOP
        DBMS_OUTPUT.PUT_LINE(V_R_DATES.CONTRACT_ID);
        DBMS_OUTPUT.PUT_LINE(V_R_DATES.MANUAL_DATE);

    BEGIN
    FOR V_EMPLOYEES IN (SELECT * FROM HRIS_CUST_CONTRACT_EMP WHERE CONTRACT_ID=V_CONTRACT_ID AND MONTH_CODE_ID=V_MONTH_CODE_ID) LOOP
    INSERT INTO HRIS_CUST_CONTRACT_ATTENDANCE (CONTRACT_ID,ATTENDANCE_DT,EMPLOYEE_ID,MONTH_CODE_ID,IN_TIME,OUT_TIME) VALUES (V_CONTRACT_ID,V_R_DATES.MANUAL_DATE,V_EMPLOYEES.EMPLOYEE_ID,V_MONTH_CODE_ID,V_EMPLOYEES.START_TIME,V_EMPLOYEES.END_TIME);
    END LOOP;
...
BEGIN
  ...
  --
  SELECT MIN(TO_DATE(TO_CHAR(A.ATTENDANCE_DT,'DD-MON-YYYY')||' '||TO_CHAR(A.ATTENDANCE_TIME,'HH:MI AM'),'DD-MON-YYYY HH:MI AM')) AS IN_TIME
  INTO V_IN_TIME
  FROM HRIS_ATTENDANCE A
  ...
  AND ADM.PURPOSE           ='IN';
  --
  SELECT MAX(TO_DATE(TO_CHAR(A.ATTENDANCE_DT,'DD-MON-YYYY')||' '|| TO_CHAR(A.ATTENDANCE_TIME,'HH:MI AM'),'DD-MON-YYYY HH:MI AM')) AS IN_TIME
  INTO V_OUT_TIME
  FROM HRIS_ATTENDANCE A
  LEFT JOIN HRIS_ATTD_DEVICE_MASTER ADM
  ON (A.IP_ADDRESS          =ADM.DEVICE_IP)
...
BEGIN
  ...
  LOOP
    ...
    END;
    UPDATE HRIS_EMPLOYEE_LEAVE_ASSIGN
    SET BALANCE       = TOTAL_DAYS - (V_TOTAL_NO_OF_DAYS+V_TOTAL_NO_OF_PENALTY_DAYS)
    WHERE EMPLOYEE_ID = leave_assign.EMPLOYEE_ID
    AND LEAVE_ID      = leave_assign.LEAVE_ID;
  END LOOP;
END;/
...
BEGIN
  ...
  LOOP
    ...
    AND LEAVE_ID            = leave.LEAVE_ID
    AND FISCAL_YEAR_MONTH_NO=leave.FISCAL_YEAR_MONTH_NO;
  END LOOP;
END;/
...
END;/
  ...
  BEGIN
    ...
    IF V_IS_MONTHLY = 'N' THEN
      ...
      END IF;
      IF :old.STATUS !='AP' AND :new.STATUS ='AP' THEN
        UPDATE HRIS_EMPLOYEE_LEAVE_ASSIGN
        SET BALANCE       = BALANCE -V_BALANCE
        WHERE EMPLOYEE_ID = :new.EMPLOYEE_ID
        AND LEAVE_ID      =:new.LEAVE_ID;
      ELSIF :old.STATUS   ='AP' AND :new.STATUS IN( 'C','R') THEN
        UPDATE HRIS_EMPLOYEE_LEAVE_ASSIGN
        SET BALANCE       = BALANCE +V_BALANCE
        WHERE EMPLOYEE_ID = :new.EMPLOYEE_ID
        AND LEAVE_ID      =:new.LEAVE_ID;
